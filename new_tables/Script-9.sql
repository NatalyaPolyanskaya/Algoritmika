DROP VIEW IF EXISTS az_oper_rep.az_codehour_new_students;
CREATE OR REPLACE VIEW az_oper_rep.az_codehour_new_students AS
WITH s2 AS (
    SELECT
        ...
        utis_code AS codehour_utis_code
...
sol AS (
    ...
    GROUP BY student_id, course_id, lesson_id 
)
SELECT
    s2.*,
    ...
    sol2.lesson_id AS codehour_lesson_id, 
    ...
    CASE
        WHEN cl.is_bonus = 0 THEN 'Обычный'
        WHEN cl.is_bonus = 1 THEN 'Бонусный'
        ELSE 'Неизвестно'
...
LEFT JOIN raw_lms.course_lesson cl ON cl.course_id = gc.course_id AND cl.lesson_id = sol2.lesson_id; 
